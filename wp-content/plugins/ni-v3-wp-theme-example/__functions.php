...
global $logo;
  ...
  * sourced from admin/class-related-posts-plus-admin.php $Related_Posts_Plus_Admin->enqueue_styles()
**/
...
function loadMyScripts() {
	if ( !is_admin() ) {
		// Register CSS
		wp_enqueue_style( 'next-investors', get_bloginfo( 'template_url' ) . '/assets/next-investors.css', null, '1.0.11', 'all' );
		wp_enqueue_style( 'stylesheet', get_bloginfo( 'template_url' ) . '/style.css', null, '1.0.0', 'all' );
		wp_enqueue_script( 'tether', get_bloginfo( 'template_url' ) . '/assets/tether.min.js', '1.0.4' );
		wp_enqueue_script( 'core', get_bloginfo( 'template_url' ) . '/assets/core.js', array('jquery'), '1.0.13', true );
	}
}
...
/*-- HIDE PARENT DROPDOWN IN CATEGORIES (START) --*/
...
/*-- HIDE PARENT DROPDOWN IN CATEGORIES (END) --*/
...
/* -- ADD BLOG ID TO BODY CLASSES (START) -- */
...
/* -- ADD BLOG ID TO BODY CLASSES (END) -- */
...
function add_paragraph_below_submit( $button, $form ) {
    ...
    return $button .= '<p class="mt-5 disclaimer-rb">Joining Raisebook will give you free access to opportunities not normally available to general retail investors â€“ however you must qualify as a sophisticated investor under Section 708 of the Corporations Act. These opportunities are as diverse as stock placements, seed capital raisings, IPOs, options underwritings. Plus a whole host of other high risk, high reward investment opportunities not available to the general public (careful â€“ this stuff is high risk!).</p>';
}
...
//add_filter( 'gform_field_content_49', 'clean_render', 10, 5 );
function clean_render( $field_content, $field ) {
    if ( $field->cssClass == 'email' ) {
